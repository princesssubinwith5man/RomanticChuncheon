...
cred = credentials.Certificate('princesssuvinand6dwarf-firebase-adminsdk-ef7wr-f80d2d8574.json')
firebase_admin.initialize_app(cred,{
    'databaseURL' : 'https://princesssuvinand6dwarf-default-rtdb.asia-southeast1.firebasedatabase.app/'
})
dir = db.reference('춘천시')
...
for dong in range(40):
    dong_str = str(dong + 1)
    dir = db.reference('춘천시/'+ dong_str)
    select = Select(driver.find_element_by_id("fc-sch-area"))
    select.select_by_index(dong_str)  # 동 선택
    driver.find_element_by_xpath(
        '/html/body/div[2]/div/div[2]/div/div[1]/div[1]/fieldset/div[1]/div[2]/button').click()  # 검색버튼 클릭
    ...
    for page in range(max_page_num):
        ...
        for x in range(12):
            ...
            dir = db.reference('춘천시/'+ dong_str + '/' + name)
            jsonResult = {
                "name": name,
                "sector": sector,
                "telnum": telnum,
            ...
            dir.update({"name": name})
            dir.update({"sector": sector})
            dir.update({"telnum": telnum})
            dir.update({"address": address})
            print(jsonResult)
            # 파일 저장
